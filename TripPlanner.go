...
func PlanTrips(w http.ResponseWriter, req *http.Request, p httprouter.Params) {
	...
	NearestLocation := getShortestRoute(request.Starting_from_location_id,request.Location_ids)
	response.Best_route_location_ids= append(response.Best_route_location_ids,NearestLocation)
	NewArray := RemoveFromArray(NearestLocation,request.Location_ids)
	
	for j:=0;len(response.Best_route_location_ids)!= len(request.Location_ids);j++{
		NewNearestLocation := getShortestRoute(request.Location_ids[j],NewArray)
